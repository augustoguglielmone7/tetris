...
export class Board {
    ...
    // Agrega una pieza al tablero si todas sus celdas son válidas
    public addPiece(cells: Cell[]): boolean {

        const canAddPiece = cells.every(cell =>
            this.isInsideBounds(cell) &&
    ...
    // Comprueba si una pieza puede colocarse
    // en una determinada fila y columna
    public canPlacePiece(
        ...
        return cells.every(cell => {
            ...
            return this.isInsideBounds(boardCell) &&
                !this.isOccupied(boardCell);
        });
    }

    // Comprueba si una celda ya está ocupada
    public isOccupied(cell: Cell): boolean {
        return this.occupiedCells.has(
    ...
    // y baja las filas que están arriba
